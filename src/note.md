...
> 说明：这是你要求的完整、可复习的设计报告（Markdown 源）。包含顶层 `cpu_top` 和所有子模块的设计说明、接口、时序语义、关键实现决策、测试/仿真要点以及扩展建议。把下面内容保存为 `.md` 文件即可日后查阅。
...
> 结束语：
> 这份 Markdown 文档汇总并系统化了之前我们讨论与实现的全部模块、接口、时序与工程考量。保存后你可以在未来快速回顾模块职责、互连方式与调试要点。若需要我可以把这份文档导成 PDF、或加入图形化数据流图、或生成适用于 Git 仓库的 README 与目录结构。
